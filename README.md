...
1. See tutorials for basic introduction to capabilities and code examples: `documentation/tutorials/`
...
# Here, metal oxidation state and spin state, methods for evaluating complexes during construction, #
...
"parameters": {
    ...
    # Method parameters.
    "full_method": "GFN2-xTB", # Which  method to use for final cleaning/evaluating conformers. 
    ...
    # Additionally, it is possible to use "UFF" - which is extremely fast. Though it is recommend to perform an XTB-level optimization
    ...
    "ff_preopt":False, # Whether to force force field (FF) pre-optimization of fully-built complexes before final xtb evaluation.
    ...
    "n_symmetries": 10, # Total metal-center symmetries to build, NSymmetries should be >= n_conformers
    "relax": True, # Perform final geometry relaxation of assembled complexes
    ...
    "seed": None, # If a seed is passed (int/float) use it to initialize np.random.seed for reproducibility.
    ...
    "ase_atoms_db_name": 'architector_ase_db_{uid}.json', # Possible to name the database filename
