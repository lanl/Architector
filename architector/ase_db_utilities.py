...
def merge_JsonFiles(filenamelist, outfname="compiled.json", maxforce=300):
    ...
    for i, f1 in tqdm(enumerate(filenamelist), total=len(filenamelist)):
        ...
        try:
            ...
            for key, val in data.items():
                try:
                    maxf = np.linalg.norm(
                        np.array(val["forces"]["__ndarray__"][2]).reshape(
                            val["forces"]["__ndarray__"][0]
                        ),
                        axis=1,
                    ).max()
...
def convert_arrays_to_npz(
    ...
    for row in tqdm(db.select(), total=total):
        ...
        try:
            ...
            result_dict = {
                ...
                - np.sum(
                    [
                        io_ptable.xtb_single_atom_ref_es[sym] * counts[i]
            ...
            distmat = (
                row.toatoms().get_all_distances()
                + np.eye(len(row.numbers)) * mindist_cutoff * 2
            )
    ...
    atomization_array = np.array(
        [record["atomization_energy"] for record in record_list]
    )
