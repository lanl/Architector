...
def vibration_analysis(
    ...
    if project_out_rot_trans:  # See https://gaussian.com/vib/
        ...
        r_COM = (masses.reshape(-1, 1) * positions).sum(axis=0) / (
            masses.sum()
        )
        ...
        for i, centered_posit in enumerate(centered):
            I[0, 0] += masses[i] * (
                centered_posit[1] ** 2 + centered_posit[2] ** 2
            )
            ...
            I[1, 1] += masses[i] * (
                centered_posit[0] ** 2 + centered_posit[2] ** 2
            )
            I[2, 2] += masses[i] * (
                centered_posit[0] ** 2 + centered_posit[1] ** 2
            )
        ...
        for i, mass in enumerate(masses):
            ...
            for j in range(3):
                D[3 * i + j, 3] = mass_12 * (
                    np.dot(centered[i], X[1]) * X[j, 2]
                    - np.dot(centered[i], X[2]) * X[j, 1]
                )
                D[3 * i + j, 4] = mass_12 * (
                    np.dot(centered[i], X[2]) * X[j, 0]
                    - np.dot(centered[i], X[0]) * X[j, 2]
                )
                D[3 * i + j, 5] = mass_12 * (
                    np.dot(centered[i], X[0]) * X[j, 1]
                    - np.dot(centered[i], X[1]) * X[j, 0]
                )
    ...
    unit_conversion = (
        ase.units._hbar * ase.units.m / np.sqrt(ase.units._e * ase.units._amu)
    )
    ...
    if mode_type == "direct_eigen_vectors":  # Unitless
        ...
        mode_type == "mass_weighted_unnormalized"
    ):  # Mass weighted has units of 1/sqrt(MW)
