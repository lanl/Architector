...
def center_molecule_gen_grid(mol, parameters={}):
    ...
    maxs = (
        new_coords.max(axis=0) + parameters["species_grid_pad"]
    )  # Add padding
...
def species_generate_get_ref_params(
    species_id, parameters={}, main_molecule=False, intermediate=False
):
    ...
    if isinstance(species_id, str):
        ...
        if ("TRIPOS" in species_id) or (
            ...
            try:
                ...
                outdict.update(
                    {"uhf": smi_obmol.GetTotalSpinMultiplicity() - 1}
                )
            except:
                outdict = {"charge": species.charge, "uhf": species.uhf}
    ...
    if parameters.get("species_location_method", "default") not in ["random", "spiral"]:
        ...
        species = calc.mol
        outdict["species_dipole"] = species.ase_atoms.calc.results.get(
            "dipole", np.zeros(3)
        )
        outdict["species_dipole_mag"] = np.linalg.norm(
            outdict.get("species_dipole", None)
        )
        outdict["species_charges"] = species.ase_atoms.calc.results.get(
            "charges", np.zeros(len(calc.mol.ase_atoms))
        )
        outdict["energy"] = species.ase_atoms.calc.results.get(
            "energy", 100000
        )
        outdict["forces"] = species.ase_atoms.calc.results.get(
            "forces", np.zeros((len(calc.mol.ase_atoms), 3))
        )
    else:
        calc = CalcExecutor(
            species,
...
def decide_new_species_location(mol, species,
    ...
    dist_grid_molecule = np.linalg.norm(
        mol.ase_atoms.get_positions()[:, None, :] - grid[None, :, :], axis=-1
    )
    upper_radvect = []
    lower_radvect = []
    for z in mol.ase_atoms.get_atomic_numbers():
        grid_radii = parameters.get("species_grid_rad_scale", 1) * (
            io_ptable.rvdw[z] + spec_rad
        )
    ...
    upper_inds = np.where(
        np.any(np.less_equal(dist_grid_molecule, upper_radvect), axis=0)
    )[0]
    lower_inds = np.where(
        np.any(np.less_equal(dist_grid_molecule, lower_radvect), axis=0)
    )[0]
    ...
    if parameters["species_location_method"] == "random":
        ...
        eq = (
            ...
            / (4 * np.pi * units._eps0)
            * spec_charge
            * mol.param_dict["species_charges"]
            / (possible_grid_dists)
        )  # kqQ/r
        eq = np.sum(eq, axis=1)
        if parameters["debug"]:
        ...
        isinstance(
            parameters["targeted_indices_close"], (list, tuple, np.ndarray)
        )
    ):
        target_inds = parameters["targeted_indices_close"]
        upper_inds_target = np.where(
            np.less_equal(
                dist_grid_molecule[target_inds[0]],
                upper_radvect[target_inds[0]],
            )
        )[
            0
        ]  # Get grid points close to only this atom
        lower_inds_target = np.where(
            np.less_equal(
                dist_grid_molecule[target_inds[0]],
                lower_radvect[target_inds[0]],
            )
        )[
            0
        ]  # Get grid points close to only this atom
...
def add_species(init_mol, species, parameters={}, species_index=None):
    ...
    for i, r in enumerate(rotations):  # Test all rotations using 'GFN-FF'
        ...
        if tmp_mol.dists_sane:
            ...
            if parameters["species_location_method"] in ["default",
                ...
                target_inds = parameters["targeted_indices_close"]
                targeted_dist = calc.mol.ase_atoms.get_distance(
                    target_inds[0], len(init_mol.ase_atoms) + target_inds[1]
                )
                obj = (
                    calc.energy / targeted_dist
                )  # Both lower energy (-VE), lower distance (+VE)
            if obj < best_obj:
    ...
    if out_rotation is not None:
        good = True
        newmol = species_generate_get_ref_params(
            out_rotation,
            parameters=parameters,
            main_molecule=True,
            intermediate="main",
        ...
        if parameters["debug"]:
            print(
                "None of the Ligands passed the calculator addition - skipping ONE."
            )
        good = False
        newmol = tmp_mol
    return newmol, good
...
def add_non_covbound_species(mol, parameters={}):
    ...
    if species_list is not None:
        species_list = [
            io_ptable.ligands_dict.get(x, {"smiles": x})["smiles"]
            for x in species_list
        ]
    elif (species_smiles is not None) and (n_species is not None):
        species_list = [
            io_ptable.ligands_dict.get(x, {"smiles": x})["smiles"]
            for x in [species_smiles] * n_species
        ]
    else:
        raise ValueError(
    ...
    for j in range(n):
        ...
        if params.get("debug", False):
            print(
                "Doing all species random addition {} of {}".format(j + 1, n)
            )
        init_mol = species_generate_get_ref_params(
            mol, parameters=params, main_molecule=True
        )
        good = True
        for i, spec in enumerate(species_list):
            if params["debug"]:
                print(
                    "Adding species {} of {}.".format(i + 1, len(species_list))
                )
                print(
                    species_dict[spec].write_mol2(
                        "cool_species", writestring=True
                    )
                )
        ...
        if (
            params.get("species_relax", True)
            and (not params.get("species_intermediate_relax", False))
            and good
        ):
            out_mol = species_generate_get_ref_params(
                init_mol, parameters=params, main_molecule=True
            )
