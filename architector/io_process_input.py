...
def assign_ligType_default(
    ...
    for ring in rings:
        if (
            all(ring.IsInRing(int(x + 1)) for x in ligcoords)
            and (len(ligcoords) > 2)
            and (ring.IsAromatic())
        ):
            is_cp = True
    total_edge_bound = (
        0  # Check for coordination atoms that are neighbors of each other.
    )
    ...
    if is_cp:
        ...
        tcore_geo_class.get_lig_ref_inds_dict(metal, tolerance=20)
        ...
        while not exitloop:
            ...
            for ligtype in possible_Ligtypes:
                ...
                best_dict = tcore_geo_class.best_liglist_geos.get(
                    ligtype, False
                )
                if isinstance(best_dict, dict):
                    ...
                    conf, minval, sane, _, _, _, _ = (
                        io_lig.get_aligned_conformer(
                            ligsmiles,
                            tmp_ligList,
                            corecoordList,
                            metal=tmetal,
                    ...
                    if sane:
                        ...
                        conf = conf + Atoms(
                            symbols=[metal], positions=[(0, 0, 0)]
                        )
                        conformers[-1] = conf
            save_inds = [i for i, x in enumerate(possible_saves) if x]
            conformers = [
                x for i, x in enumerate(conformers) if (i in save_inds)
            ]
            tpossible_Ligtypes = [
                x for i, x in enumerate(possible_Ligtypes) if (i in save_inds)
            ]
        ...
        for i, conf in enumerate(conformers):
            ltype, minloss, confidence, _, _ = io_core.classify_ligtype(
                conf, ligcoords
            )
...
def assign_ligType_bruteforce(
    ...
    for ring in rings:
        if (
            all(ring.IsInRing(x + 1) for x in ligcoords)
            and (len(ligcoords) > 2)
            and (ring.IsAromatic())
        ):
            is_cp = True
    ...
    if is_cp:
        ...
        raise ValueError(
            "Error: Edge ligands with more than 2 coordination sites are yet-unsupported in Architector."
        )
    elif total_edge_bound == 1:
        return "edge_bi_cis"
    else:
        ...
        if debug:
            print(
                "ligType not specified for {} - testing ligand placement to determine ligType!".format(
                    ligsmiles
                )
            )
            print(
                "Warning: can take a while depending on the size of the ligand."
            )
        ...
        for ligtype in possible_Ligtypes:
            ...
            for coreType in core_geo_class.liglist_geo_map_dict[ligtype]:
                ...
                # Pick first one.
                core_cons = core_geo_class.liglist_geo_map_dict[ligtype][
                    coreType
                ][0]
                tmp_ligList = []
                for i in range(len(ligcoords)):
        ...
        if not possible_saves[min_ind]:  # Try with larger radii
            tcovrad_metal = (
                io_ptable.rcov1[io_ptable.elements.index(tmetal)] * 1.3
            )
            tvdwrad_metal = (
                io_ptable.rvdw[io_ptable.elements.index(tmetal)] * 1.3
            )
            rot_vals = []
            possible_saves = []
            if debug:
            ...
            for ligtype in possible_Ligtypes:
                ...
                for coreType in core_geo_class.liglist_geo_map_dict[ligtype]:
                    ...
                    _, minval, sane, _, _, _, _ = io_lig.get_aligned_conformer(
                        ...
                        corecoordList,
                        metal=tmetal,
                        covrad_metal=tcovrad_metal,
                        vdwrad_metal=tvdwrad_metal,
                    )
                    if sane:
                        rot_vals[-1].append(minval)
                        possible_saves[-1] = True
        ...
        if not possible_saves[min_ind]:  # Try with smaller radii
            tcovrad_metal = (
                io_ptable.rcov1[io_ptable.elements.index(tmetal)] * 0.8
            )
            tvdwrad_metal = (
                io_ptable.rvdw[io_ptable.elements.index(tmetal)] * 0.8
            )
            rot_vals = []
            possible_saves = []
            if debug:
        ...
        if possible_saves[min_ind]:
            if debug:
                print(
                    "Assigning lig {} to ligType {}!".format(
                        ligsmiles, possible_Ligtypes[min_ind]
                    )
                )
            return possible_Ligtypes[min_ind]
        else:
            raise ValueError(
                "Cannot assign lig {} to any ligType!".format(ligsmiles)
            )


def assign_ligType_similarity(
    ligsmiles,
    ligcoords,
    metal,
    covrad_metal=None,
    ...
    if debug:
        ...
        print(
            "Possibly slow - using similarity to training data for assignment!"
        )
    OBmol = io_obabel.get_obmol_smiles(ligsmiles)
    tmetal, _ = io_ptable.convert_actinides_lanthanides(metal)
    rings = OBmol.GetSSSR()
    ...
    if is_cp:
        ...
        for _, row in io_ptable.ligands_dict.items():
            ...
            obmollig = io_obabel.get_obmol_smiles(
                row["smiles"], addHydrogens=True, neutralize=False, build=False
            )
        ...
        for _, row in ligrefdf.iterrows():
            ...
            _, anums, _ = io_obabel.get_OBMol_coords_anums_graph(
                refobmol, return_coords=False, get_types=False
            )
            ...
            tanimoto_sim = (
                refpf | ligfp
            )  # Returns tanomoto similarity between two fingerprints
...
def inparse(inputDict):
    ...
    if (
        ("core" in inputDict)
        and (("ligands" in inputDict) or ("ligandList" in inputDict))
    ) or ("mol2string" in inputDict):
        if ("core" in inputDict) and (
            ("ligands" in inputDict) or ("ligandList" in inputDict)
        ):
            ...
            if "metal" in coreDict:
                ...
                if ("[" not in coreDict["smiles"]) or (
                    "]" not in coreDict["smiles"]
                ):
                    print(
                        "Warning: attempting to add parenthesis to this smiles string: ",
                        coreDict["smiles"],
                    )
            ...
            if "coordList" in coreDict:
                if isinstance(coreDict["coordList"], list):
                    ...
                    core_geo_class = io_core.Geometries(
                        usercore=coreDict["coordList"]
                    )
                    coreTypes = ["user_core"]
                    skip = True
                elif (coreDict["coordList"] is None) or isinstance(
                    coreDict["coordList"], bool
                ):
                    skip = False
                else:
                    raise ValueError(
                        'Unrecognized type passed to inputDict["core"]["coordList"] - need list or None/bool.'
                    )
            if ("coreType" in coreDict) and (not skip):
                if isinstance(coreDict["coreType"], list):
                    ...
                    for x in coreDict["coreType"]:
                        if x in core_geo_class.geometry_dict:
                            ...
                            print(
                                "{} not in known coreTypes. Skipping.".format(
                                    x
                                )
                            )
                            print(
                                "Known coreTypes: ",
                                core_geo_class.geometry_dict.keys(),
                            )
                elif (coreDict["coreType"], str):
                    core_geo_class = io_core.Geometries()
                    if (
                        coreDict["coreType"]
                        in core_geo_class.geometry_dict.keys()
                    ):
                ...
                elif (coreDict["coreType"] is None) or isinstance(
                    coreDict["coreType"], bool
                ):
                    skip = False
                else:
                    raise ValueError(
                ...
                if isinstance(coreDict["coreCN"], list):
                    ...
                    for x in coreDict["coreCN"]:
                        if x in core_geo_class.cn_geo_dict.keys():
                            ...
                            print(
                                "Valid coreCNs are: ",
                                core_geo_class.cn_geo_dict.keys(),
                            )
                    ...
                    core_geo_class = io_core.Geometries()
                    print(
                        "At this point not sure what is up. Defaulting to coreCN=6."
                    )
                    coreTypes += core_geo_class.cn_geo_dict[6]
                else:
                    raise ValueError(
                        'Unrecognized type passed to inputDict["core"]["coreCN"] - need list/int/float/None/bool.'
                    )
            ...
            vdwrad_metal = None
            if (
                "parameters" in newinpDict
            ):  # Load input parameters as changes to the defaults.
            ...
            core_geo_class.get_lig_ref_inds_dict(
                metal, coreTypes, rcovmetal=covrad_metal
            )  # Calculate core references -> rescaled
            ...
            if isinstance(ligList, list):
                for ligDict in ligList:
                    ...
                    if isinstance(ligDict, dict):
                        if (
                            ("smiles" in ligDict)
                            and ("coordList" in ligDict)
                            and ("ligType" in ligDict)
                        ):
                            newdict.update({"smiles": ligDict["smiles"]})
                            if (ligDict["smiles"] == "[H-]") or (
                                ligDict["smiles"] == "[O-2]"
                            ):  # Handle hydrides
                            ...
                            if isinstance(ligDict["ligType"], list):
                                ...
                                for lt in ligDict["ligType"]:
                                    if (
                                        (
                                            lt
                                            in core_geo_class.liglist_geo_map_dict.keys()
                                        )
                                        or (lt == "mono")
                                        or ("edge" in lt)
                                    ):
                                        ...
                                        raise ValueError(
                                            "Error: {} ligType not recognized!".format(
                                                lt
                                            )
                                        )
                                ...
                                print(
                                    "Error: {} ligand type not recognized!".format(
                                        ligDict["ligType"]
                                    )
                                )
                                print(
                                    "Valid ligand types: ",
                                    core_geo_class.liglist_geo_map_dict.keys(),
                                    " + mono",
                                )
                                raise ValueError
                            if (
                                "edge" in newdict["ligType"]
                            ):  # Duplicate test of edge type ligands in case somethin unsuppored requested.
                                lt = assign_ligType_default(
                                    core_geo_class,
                                    ligDict["smiles"],
                                    ligDict["coordList"],
                                ...
                                newdict.update({"ligType": lt})
                            if isinstance(
                                ligDict["coordList"][0], (int, float)
                            ):
                                newdict.update(
                                    {
                                        "coordList": [
                                            int(x)
                                            for x in ligDict["coordList"]
                                        ]
                                    }
                                )
                            elif isinstance(ligDict["coordList"][0], list):
                                newdict.update(
                                    {"coordList": ligDict["coordList"]}
                                )
                            else:
                                raise ValueError(
                                    "Error: {} ligand coordList not recognized!".format(
                                        ligDict["coordList"]
                                    )
                                )
                        elif ("smiles" in ligDict) and (
                            "coordList" in ligDict
                        ):
                            ...
                            if isinstance(ligDict["coordList"][0], list):
                                ...
                                len(ligDict["coordList"]) > 1
                            ):  # Handle other cases with bruteforce assignemnt of ligandType!
                                newdict.update(
                                    {"coordList": ligDict["coordList"]}
                                )
                                bruteforce = True
                                for (
                                    ...
                                    newliglist
                                ):  # Check other ligands if ligType already assigned.
                                    if (
                                        tdict["smiles"] == newdict["smiles"]
                                    ) and (
                                        sorted(tdict["coordList"])
                                        == sorted(newdict["coordList"])
                                    ):
                                        newdict.update(
                                            {"ligType": tdict["ligType"]}
                                        )
                                        bruteforce = False
                                        break
                                if bruteforce:
                                    ...
                                    if (
                                        "lig_assignment"
                                        in newinpDict["parameters"]
                                    ):
                                        if (
                                            newinpDict["parameters"][
                                                "lig_assignment"
                                            ]
                                            == "default"
                                        ):
                                        ...
                                        tmp_ligType = assign_ligType_default(
                                            core_geo_class,
                                            newdict["smiles"],
                                            newdict["coordList"],
                                            metal,
                            ...
                            "smiles" in ligDict
                        ):  # Handle simple ligands without needed specs.
                            if ligDict["smiles"] == "[H-]":  # Handle hydrides
                                ...
                                raise ValueError(
                                    "This smiles is not recognized - need coordList/ligType assignment {}!".format(
                                        ligDict["smiles"]
                                    )
                                )
                        elif (
                            "name" in ligDict
                        ):  # Read from io_ptable reference ligands!
                            if (
                                ligDict["name"].lower()
                                in io_ptable.ligands_dict
                            ):
                                newdict = copy.deepcopy(
                                    io_ptable.ligands_dict[
                                        ligDict["name"].lower()
                                    ]
                                )
                            else:
                                raise ValueError(
                                    """This ligand's {} name is not recognized. See architector.io_ptable for reference ligands.
                                 Note that this should be case-insensitive. """.format(
                                        ligDict["name"]
                                    )
                                )
                        else:
                            raise ValueError(
                                'Need at least "smiles" and "coordList" for each ligand or "name"!'
                            )
                    elif isinstance(
                        ligDict, str
                    ):  # Add ligand from the reference
                        if ligDict.lower() in io_ptable.ligands_dict:
                            newdict = copy.deepcopy(
                                io_ptable.ligands_dict[ligDict.lower()]
                            )
                        else:
                            raise ValueError(
                                """This ligand's {} name is not recognized. See architector.io_ptable for reference ligands.
                    ...
                    if isinstance(ligDict, dict):
                        if "functionalizations" in ligDict:
                            tmpOBmol = io_obabel.get_obmol_smiles(
                                newdict["smiles"],
                                functionalizations=ligDict[
                                    "functionalizations"
                                ],
                            )
                        ...
                        if "ca_metal_dist_constraints" in ligDict:
                            newdict.update(
                                {
                                    "ca_metal_dist_constraints": ligDict[
                ...
                raise ValueError(
                    "inputDict['ligands'] must be a list of dictionaries currently."
                )
        ####################################
        ...
        ####################################
            ...
            if mol.uhf is not None:
                ...
                if isinstance(newinpDict["parameters"], dict):
                    ...
                    newinpDict["parameters"]["full_spin"] = mol.uhf
            if (
                mol.xtb_uhf is not None
            ):  # Currently default to XTB spin -> xtb backend makes more sense
                if "parameters" not in newinpDict:
                    newinpDict["parameters"] = dict()
                if isinstance(newinpDict["parameters"], dict):
            ...
            metal_inds = [
                i
                for i, x in enumerate(mol.ase_atoms.get_chemical_symbols())
                if x in io_ptable.all_metals
            ]
            if (
                len(metal_inds) == 1
            ):  # Only one metal - perform ligand breakdown to find all ligands.
                ##### Core Preprocessing ####
                ...
                if not isinstance(newinpDict["parameters"], dict):
                    newinpDict["parameters"] = dict()
                newinpDict["core"]["coreCN"] = len(
                    np.nonzero(np.ravel(mol.graph[met_ind]))[0]
                )
                core_geo_class = io_core.Geometries()
                ...
                if (
                    ...
                    if isinstance(newinpDict["parameters"], dict):
                        if "covrad_metal" in newinpDict["parameters"]:
                            covrad_metal = newinpDict["parameters"][
                                "covrad_metal"
                            ]
                        if "vdwrad_metal" in newinpDict["parameters"]:
                            vdwrad_metal = newinpDict["parameters"][
                                "vdwrad_metal"
                            ]
                ...
                lig_smiles, coord_atoms = io_obabel.obmol_lig_split(
                    inputDict["mol2string"]
                )
                newliglist = list()
                for i, lig_smiles in enumerate(lig_smiles):
                    ...
                    if (tligdict["smiles"] == "[H-]") or (
                        ...
                        for (
                            ...
                            if (tdict["smiles"] == tligdict["smiles"]) and (
                                sorted(tdict["coordList"])
                                == sorted(tligdict["coordList"])
                            ):
                        ...
                        if bruteforce:
                            ...
                            if "lig_assignment" in newinpDict["parameters"]:
                                if (
                                    newinpDict["parameters"]["lig_assignment"]
                                    == "bruteforce"
                                ):
                                    tmp_ligType = assign_ligType_bruteforce(
                                        core_geo_class,
                                        tligdict["smiles"],
                                        tligdict["coordList"],
                                        metal,
                                    ...
                                    tmp_ligType, _ = assign_ligType_similarity(
                                        tligdict["smiles"],
                                        tligdict["coordList"],
                                        metal,
                                        covrad_metal=covrad_metal,
                                        full_diff_factor=0.8,
                                    )
                            else:
                                tmp_ligType, _ = assign_ligType_similarity(
                                    tligdict["smiles"],
                ...
                raise ValueError(
                    "Either less than 1 or more than 1 metal in this mol2string passed."
                )
        ...
        for coreType in coreTypes:
            good = True
            for lig in newinpDict[
                "ligands"
            ]:  # Check that all ligands can map to this geometry
                if lig["ligType"] == "mono":
                    good = True
                    ...
                    for lt in lig["ligType"]:
                        if (
                            coreType
                            in core_geo_class.liglist_geo_map_dict[lt].keys()
                        ):
                            good = True
                elif (
                    coreType
                    not in core_geo_class.liglist_geo_map_dict[
                        lig["ligType"]
                    ].keys()
                ):
                    good = False
        ...
        if len(coreTypes_run) == 0:
            print(
                "Warning: No structures generated! No Coretype/ligand type match."
            )
        ...
        if len(coreTypes) == 0:
            print(
                "Warning: No structures generated! Still couldn't map the ligands to the core"
            )
        ...
        default_parameters = {
            ...
            "ase_atoms_db_name": "architector_{}_ase_db_".format(
                newinpDict["core"]["metal"]
            )
            + u_id
            + ".json",
            ...
            "species_list": ["water"]
            * 3,  # Pass a list of species (preferred)
        ...
        if (newinpDict["core"]["metal"] in io_ptable.actinides) and (
            ("xtb" in outparams.get("species_method", "none").lower())
            or ("uff" in outparams.get("species_method", "none").lower())
        ):
        ...
        if outparams["force_generation"]:
            # Graph Sanity cutoff for imposed molecular graph represents the maximum elongation of bonds
            # rcov1*full_graph_sanity_cutoff is the maximum value for the bond lengths.
            outparams["assemble_smallest_dist_cutoff"] = (
                0.7  # Force this to be tighter if no optimization done.
            )
            outparams["full_smallest_dist_cutoff"] = (
                0.7  # Force this to be tighter if no optimization done.
            )
        ...
        if isinstance(outparams["fill_ligand"], int):
            outparams["fill_ligand"] = newinpDict["ligands"][
                outparams["fill_ligand"]
            ]
        elif isinstance(outparams["fill_ligand"], str):
            if outparams["fill_ligand"] in io_ptable.ligands_dict:
                outparams["fill_ligand"] = io_ptable.ligands_dict[
                    outparams["fill_ligand"]
                ]
            else:
                raise ValueError(
                    "Unrecognized ligand name: {}".format(
                        outparams["fill_ligand"]
                    )
                )
        ...
        if isinstance(outparams["secondary_fill_ligand"], str):
            if outparams["secondary_fill_ligand"] in io_ptable.ligands_dict:
                outparams["secondary_fill_ligand"] = io_ptable.ligands_dict[
        ...
        if outparams["metal_ox"] is None:
            if metal in io_ptable.metal_charge_dict:
                ...
                outparams["metal_ox"] = [
                    x.oxidation_state
                    for x in elem._oxidation_states
                    if (x.category == "main") and (x.oxidation_state > 0)
                ][0]
        if outparams["metal_spin"] is None:
            if outparams["metal_ox"] != io_ptable.metal_charge_dict.get(
                metal, 100
            ):
                # Calculate from mendeleev reference - Generally aufbau.
                outparams["metal_spin"] = (
                    mendeleev.element(newinpDict["core"]["metal"])
                    .ec.ionize(outparams["metal_ox"])
                    .unpaired_electrons()
                )
        ...
        if outparams["alternate_metal_spin"] is None:
            if metal in io_ptable.second_choice_metal_spin_dict:
                outparams["alternate_metal_spin"] = (
                    io_ptable.second_choice_metal_spin_dict[metal]
                )

        # Connect to ase database
        outparams["ase_db_tmp_name"] = os.path.join(
            outparams["temp_prefix"], outparams["ase_atoms_db_name"]
        )
        ...
        if outparams["force_trans_oxos"]:
            coreTypes = [
                x
                for x in newinpDict["coreTypes"]
                if x in core_geo_class.trans_geos
            ]
            newinpDict["coreTypes"] = coreTypes
        ...
        if outparams["ff_preopt"]:
            ...
            outparams["assemble_sanity_checks"] = (
                False  # Allow for close/overlapping atoms
            )
            outparams["fmax"] = (
                0.2  # Slightly decrease accuracy to encourage difficult convergence
            )
    ...
    if ("core" in inputDict) and (
        ...
        if "coordList" in coreDict:
            if isinstance(coreDict["coordList"], list):
                ...
                coreTypes += core_geo_class.cn_geo_dict[
                    len(coreDict["coordList"])
                ]
                skip = True
            elif (coreDict["coordList"] is None) or isinstance(
                coreDict["coordList"], bool
        ...
        if isinstance(ligList, list):
            for ligDict in ligList:
                ...
                if isinstance(ligDict, dict):
                    if (
                        ...
                        if (ligDict["smiles"] == "[H-]") or (
                            ...
                            newinpDict["parameters"]["relax"] = False
                            # newinpDict['parameters']['metal_spin'] = 0 # Set to low_spin
                        if (ligDict["ligType"] == "mono") or (
                            isinstance(ligDict["ligType"], str)
                        ):
                            newdict.update({"ligType": ligDict["ligType"]})
                        else:
                            print(
                        ...
                        if isinstance(ligDict["coordList"][0], (int, float)):
                            newdict.update(
                                {
                                    "coordList": [
                                        int(x) for x in ligDict["coordList"]
                                    ]
                                }
                            )
                        elif isinstance(ligDict["coordList"][0], list):
                        ...
                        if isinstance(ligDict["coordList"][0], list):
                            ...
                            tmp_ligType = test_ligType_sandwich(
                                ligDict["smiles"], ligDict["coordList"]
                            )
                            newdict.update({"coordList": ligDict["coordList"]})
                            newdict.update(
                                {"ligType": tmp_ligType}
                            )  # Doesn't matter much for 2D except sandwich
                        else:
                            raise ValueError(
                                "Error: {} ligand coordList not recognized!".format(
                                    ligDict["coordList"]
                        ...
                        if ligDict["name"].lower() in io_ptable.ligands_dict:
                            newdict = copy.deepcopy(
                                io_ptable.ligands_dict[ligDict["name"].lower()]
                            )
                        else:
                            raise ValueError(
                                """This ligand's {} name is not recognized. See architector.io_ptable for reference ligands.
                        ...
                        raise ValueError(
                            'Need at least "smiles" and "coordList" for each ligand!'
                        )
                ...
                if "functionalizations" in ligDict:
                    tmpOBmol = io_obabel.get_obmol_smiles(
                        newdict["smiles"],
                        functionalizations=ligDict["functionalizations"],
                    )
                    newsmi = io_obabel.get_smiles_obmol(tmpOBmol)
                    newdict.update({"smiles": newsmi})
                newliglist.append(newdict)
        ...
        tmp_cn = int(
            np.round(
                np.mean([core_geo_class.geo_cn_dict[x] for x in coreTypes]), 0
            )
        )
        newLigInputDicts = newinpDict["ligands"].copy()

        nsand = np.sum(
            [1 for x in newLigInputDicts if (x["ligType"] == "sandwich")]
        )
        if (
            nsand > 0
            ...
            n_fill_ligs = tmp_cn - np.sum(
                [len(x["coordList"]) for x in newLigInputDicts]
            )

        n_fill_ligs_reduced = np.floor(
            n_fill_ligs / len(outparams["fill_ligand"]["coordList"])
        )
        n_fill_secondary = n_fill_ligs - (n_fill_ligs_reduced) * len(
            outparams["fill_ligand"]["coordList"]
        )

        if n_fill_ligs < 0:
            print(n_fill_ligs, newLigInputDicts)
            raise ValueError(
                "Error - the requested complex is over-coordinated!"
            )

        # Populate with Fill ligand and waters.
        elif n_fill_ligs > 0:
            for _ in range(int(n_fill_ligs_reduced)):
                newLigInputDicts.append(outparams["fill_ligand"])
            for _ in range(int(n_fill_secondary)):
                newLigInputDicts.append(outparams["secondary_fill_ligand"])
