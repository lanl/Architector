...
def get_lig_ref_df():
    ...
    filepath = os.path.abspath(
        os.path.join(__file__, "..", "data", "ligtype_angle_reference.csv")
    )
...
def get_angle(coord1, coord2, coord3):
    ...
    try:
        ...
        if np.isnan(angle):
            if np.any(
                np.isclose(
                    v1 / np.linalg.norm(v1) + v2 / np.linalg.norm(v2), 0
                )
            ):
                return 180.0
            else:
    ...
    # Catch cases where angles are too close (parallel/antiparallel)
        ...
        if np.any(
            np.isclose(v1 / np.linalg.norm(v1) + v2 / np.linalg.norm(v2), 0)
        ):
            return 180.0
        else:
            return 0.0
...
def classify_ligtype(ligand, ligcoordList):
    ...
    for ind0, ind1 in itertools.combinations(ligcoordList, 2):
        angle = get_angle(
            ligmol.ase_atoms.positions[ind0],
            np.array([0, 0, 0]),
            ligmol.ase_atoms.positions[ind1],
        )
        lig_angles.append(angle)
    lig_angles = np.array(lig_angles)[np.argsort(lig_angles)[::-1]].tolist()
...
class Geometries:
    ...
    def __init__(self, usercore=False):
        """__init__"""
        self.geometry_dict = {
            f: getattr(geo, f) for f in dir(geo) if not f.endswith("__")
        }
        ...
        self.geo_cn_dict = {k: len(v) for k, v in self.geometry_dict.items()}

        self.cn_geo_dict = {k: [] for k in set(self.geo_cn_dict.values())}
    ...
    def rescale_refgeos(self, core_atom, rcovmetal=None):
        ...
        if isinstance(rcovmetal, float):
            interatomic_dist_ideal = (
                rcovmetal + io_ptable.rcov1[io_ptable.elements.index("O")]
            )
        else:
            interatomic_dist_ideal = (
                io_ptable.rcov1[io_ptable.elements.index(core_atom)]
                + io_ptable.rcov1[io_ptable.elements.index("O")]
            )
    ...
    def get_lig_ref_inds_dict(
        ...
        for _, row in df.iterrows():
            self.ligType_cn_dict.update(
                {row["ligtype"]: int(row["denticity"])}
            )

        self.cn_ligType_dict = {
            k: [] for k in set(self.ligType_cn_dict.values())
        }
        ...
        if not isinstance(userlig, bool):
            if isinstance(userlig, (np.ndarray, list)):
                ...
                lig_angles = np.array(userlig)[np.argsort(angs)[::-1]].tolist()
                lig_angles += [0.0] * (36 - len(lig_angles))  # Pad with zeros
        ...
        for _, row in df.iterrows():
            ...
            for core_type in possible_geos:
                ...
                for j, coordat_comb in enumerate(all_coordat_combs):
                    angs = [
                        get_angle(x[0], metal_coords, x[1])
                        for x in itertools.combinations(coordat_comb, 2)
                    ]
                    ...
                    if np.all(angs <= max_angles) & np.all(angs >= min_angles):
                        ...
                        #  -> don't use these!!!
                        intercalating = check_intercalation(
                            coordat_comb, coordat_locs
                        )
                ...
                for key in outdict.keys():  # key is core_type
                    ...
                    out = out.tolist()
                    if (
                        self.best_liglist_geos.get(
                            row["ligtype"], {"loss": np.inf}
                        ).get("loss", np.inf)
                        > minloss
                    ):
