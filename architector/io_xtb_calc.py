...
def set_XTB_calc(ase_atoms, parameters=dict(), assembly=False, isCp_lig=False):
    ...
    if isCp_lig:
        ...
        calc = XTB(
            method="GFN-FF"
        )  # ,verbosity=0) # Defaul to only GFN-FF for ligand conformer relaxation.
    else:
        ...
        if len(metals) == 1:
            ...
            print(
                "No metals - continuing anyway with obviously no f in core elements."
            )

        # Handle spin / magnetism
        even_odd_electrons = (
            np.sum([atom.number for atom in ase_atoms]) - mol_charge
        ) % 2
        if (parameters["full_spin"] is not None) and (not assembly):
            ...
            if not f_in_core:
                ...
                even_odd_electrons = np.sum(
                    [atom.number for atom in ase_atoms]
                )
                even_odd_electrons = (
                    even_odd_electrons
        ...
        if assembly:
            ...
            ):  # Need to turn off charges for GFN-FF evaluation. Probably an XTB-end bug.
                ase_atoms.set_initial_charges(np.zeros(len(ase_atoms)))
                ase_atoms.set_initial_magnetic_moments(
                    np.zeros(len(ase_atoms))
                )
            calc = XTB(
                method=parameters["assemble_method"],
                solvent=parameters["solvent"],
            )
            # verbosity=0)
        else:
            if (
                parameters["full_method"] == "GFN-FF"
            ):  # Need to turn off charges for GFN-FF evaluation. Probably an XTB-end bug.
                ase_atoms.set_initial_charges(np.zeros(len(ase_atoms)))
                ase_atoms.set_initial_magnetic_moments(
                    np.zeros(len(ase_atoms))
            ...
            calc = XTB(
                method=parameters["full_method"], solvent=parameters["solvent"]
            )
            #    verbosity=0)
...
def set_XTB_calc_lig(
    ase_atoms, charge=None, uhf=None, method="GFN2-xTB", solvent="none"
):
    """set_XTB_calc
...
def xtb_relax(
    ...
    if isinstance(structure, ase.atoms.Atoms):
        ...
        mol = io_molecule.convert_io_molecule(
            structure, detect_charge_spin=detect_charge_spin
        )
...
def get_rxyz_string(ase_atoms):
    ...
    # then force
    ss += "FORCES\n"
    for symb, force in zip(
        ase_atoms.get_chemical_symbols(), ase_atoms.get_forces()
    ):
    ...
    ss += "ATOM-CHARGE-SPIN\n"
    for symb, charge, spin in zip(
        ase_atoms.get_chemical_symbols(),
        ase_atoms.get_charges(),
    ...
    ss += "PA_BINDING_ENERGY %22.14f\n" % (
        ase_atoms.get_total_energy() * eV2Hartree / len(ase_atoms)
    )
    ...
    ss += "MULTIPLICITY %d\n" % round(
        sum(ase_atoms.get_initial_magnetic_moments) + 1
    )
    # fp.write("DIFFAB %f\n" % sum(result['atomspin']))
        ...
        if io_ptable.elements.index(elem) < 87:
            ...
            if (not failed) and (energy is not None):
                if energydict[elem] > (energy):
                    energydict[elem] = energy
                    print(
                        "replaced {} with 0 charge alternate spin".format(elem)
                    )
    print("--------4---------")
    for elem, spin in io_ptable.metal_spin_dict.items():
        print(elem)
