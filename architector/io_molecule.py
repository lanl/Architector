...
def convert_io_molecule(
    ...
    if isinstance(structure, (str, ase.atoms.Atoms)):
        ...
        # mol2 filename
            ...
            and (structure.split("\n")[0].replace(" ", "").isnumeric())
            and (("FORCES" in structure) or ("ENERGY" in structure))
        ):  # RXYZ string.
...
class Molecule:
    ...
    def __init__(
        ...
        if isinstance(in_ase, ase.atoms.Atoms):
            ...
            raise ValueError(
                "Need ase.atoms.Atoms/xyz/mol2 as input for molecule class!"
            )
    ...
    def write_mol2(self, filename, writestring=False):
        ...
        if (
            ...
            and (self.uhf is not None)
            and (self.xtb_charge is not None)
            and (self.xtb_uhf is not None)
        ):
        ...
        for i, atom in enumerate(self.ase_atoms):
            ...
            ss += "{0:6d} {1:6s} {2:9.4f} {3:9.4f} {4:9.4f}   {5:6s}{6:5d} {7:5s}{8:8.4f}\n".format(
                i + 1,
                atom.symbol + str(int(atom_type_numbers[type_ind])),
                atom_coords[0],
                atom_coords[1],
                atom_coords[2],
                atom_types_mol2,
        ...
        for i, b1 in enumerate(bonds[0]):
            ...
            if b2 > b1 and not bondorders:
                ...
                ss += "{0:6d}{1:6d}{2:6d}{3:>5s}\n".format(
                    ...
                    b1 + 1,
                    b2 + 1,
                    str(self.BO_dict[(int(b1) + 1, int(b2) + 1)]),
                )
                bond_count += 1
        ...
        if hasattr(self, "cell"):
            ...
            if (
                len(self.cell) == 6
            ):  # Only abc,alphabetagamma defined (add 1 1 for space group)
                ss += "@<TRIPOS>CRYSIN\n"
        ...
        for line in s:
            ...
            if read_atoms:
                ...
                if atom_symbol1 in io_ptable.elements:
                    atom = ase.atom.Atom(
                        atom_symbol1,
                        [float(s_line[2]), float(s_line[3]), float(s_line[4])],
                    )
    ...
    def detect_charge_spin(self):
        ...
        if len(metals) > 0:
            ...
            _, _, infodict = io_obabel.obmol_lig_split(
                mol2str, return_info=True, calc_coord_atoms=False
            )
            ...
            met_syms = []
        even_odd_electrons = (
            np.sum([atom.number for atom in self.ase_atoms]) - charge
        ) % 2
        if uhf is not None:
            uhf = uhf
    ...
    def find_metals(self):
        ...
        metals = [
            i
            for i, x in enumerate(self.ase_atoms)
        ...
        if isinstance(inds, (list, np.ndarray)):
            ...
            for ind in delete_inds:
                try:
                    ...
                    print(
                        "Warning - type unknown for index {}. Need an integer got {}! This atom was not removed.".format(
                            ind, type(ind)
                        )
                    )
        else:
            print(
                "Warning - either unknown list/array of indices to delete passed {}. No atoms were removed.".format(
                    inds
                )
            )
    ...
    def create_mol_graph(self, cutoffs=True, skin=0.2, allow_mm_bonds=False):
        ...
        """
        if cutoffs:
            cutoffsvect = np.array(
                [io_ptable.rcov1[atom.number] for atom in self.ase_atoms]
            )
        metals = self.find_metals()
            ...
            if len(self.ase_atoms) > 1:  # Don't test distances for single atom
                ...
                if len(m_inds) > 1:
                    if params.get("debug", False):
                        print(
                            "Warning - Sanity check with custom covrad only for mononuclear so far. Setting Defaults for all metals."
                        )
                        multi = True
                elif len(m_inds) == 1:
                    m_ind = m_inds[0]
    ...
    def dist_sanity_checks(
        ...
        if isinstance(pair_cutoffs, dict):
            ...
            for key, val in pair_cutoffs.items():
                ...
                if newkey in new_pair_dict:
                    if params.get("debug", False):
                        print(
                            "Warning: multiple distance cutoffs passed for the same set of atom pairs"
                        )
        ...
        if run_check:
            ...
            if len(atoms) > 1:  # Don't test distances for single atom
                ...
                m_inds = [
                    i
                    for i, x in enumerate(atoms.get_chemical_symbols())
                    if x in io_ptable.all_metals
                ]
                m_ind = None
                if len(m_inds) > 1:
                            ...
                            if isinstance(pair_cutoffs, dict):
                                if new_pair_dict["type"] == "scale_covrad":
                                    ...
                                    if params.get("debug", False):
                                        ...
                                        print(
                                            'Acceptable types: "dist" or "scale_covrad!'
                                        )
                            else:
                                cutoff = smallest_dist_cutoff * (
                                    cov_radii[i] + cov_radii[j]
                                )
                            if (
                                all_dists[i, j] < cutoff
                            ):  # Check for extra-crowded metals
                                sane = False
                                smallest_dist_dict.update({"Cutoff": cutoff})
                                smallest_dist_dict.update(
                                    {
                                        (i, j): all_dists[i, j]
                                        / (cov_radii[i] + cov_radii[j])
                                    }
        ...
        self.dists_sane = sane
        self.sanity_check_dict.update(
            {
                "Smallest_Dist_Checks": smallest_dist_dict,
    ...
    def calc_suggested_spin(self, params={}):
        ...
        if params.get("full_charge", None) is not None:
            ...
            self.ase_atoms.get_initial_charges() != 0
        ):  # Prioritize ASE atoms charge
            ...
            if params.get("metal_ox", None) is not None:
                ...
                if len(metals) > 0:
                    syms = self.ase_atoms.get_chemical_symbols()
                    charge_vect[0] = np.sum(
                        [io_ptable.metal_charge_dict[syms[x]] for x in metals]
                    )
                else:
        ...
        if np.any(
            [
                True
                for x in self.ase_atoms.get_chemical_symbols()
                if x in io_ptable.heavy_metals
            ]
        ) and ((params.get("metal_ox", None) is not None)):
            xtb_charge = mol_charge + (3 - params.get("metal_ox", None))

        # Handle spin / magnetism
        even_odd_electrons = (
            np.sum([atom.number for atom in self.ase_atoms]) - mol_charge
        ) % 2
        ...
        if not np.any(
            ...
            even_odd_electrons = (
                ...
                - io_ptable.elements.index(
                    self.ase_atoms.get_chemical_symbols()[metals[0]]
                )
                + 11
                - xtb_charge
            )
    ...
    def calc_lig_angles_struct(self):
        ...
        if len(mets) == 1:
            ...
            if len(coordats) == 1:
                ...
                angs = [
                    self.ase_atoms.get_angle(x[0], mets[0], x[1])
                    for x in itertools.combinations(coordats, 2)
                ]
                angs = np.array(angs)[np.argsort(angs)[::-1]]  # Add angles
                lig_angles += angs.tolist()  # Add sorted angles as features
        else:
            print(
                "Warning: User ligand input without metal for refernce on interatomic angles. \
    ...
    def classify_metal_geo_type(self, return_result=False, debug=False):
        ...
        if len(metal_inds) > 1:  # Look at every metal center
            ...
            if len(neighs) < 13:
                ...
                metal_center_geos["classification_dict"] = {
                    ref_geo_labels[i]: mae_losses[i] for i in sort_order
                }
            else:
                metal_center_geos["metal"] = (
    ...
    def get_dists(
        ...
        if isinstance(ref_ind, str):
            if ref_ind == "metals":
                ...
                raise NotImplementedError(
                    'Not yet implemented for other keywords - only "metals".'
                )
        ...
        if (
            (len(atoms) > 0)
            or (atom_pairs is not None)
            or (atom_type_pairs is not None)
        ):
                ...
                for met in atoms:
                    ...
                    if calc_nonbonded_dists:
                        ...
                        other_close_atoms = np.array(
                            [
                                x
                                for x in other_close_atoms
                                if x not in (con_atoms.tolist() + [int(met)])
                            ]
                        )
                            ...
                            ml_dist_dicts.append(
                                {
                                    ...
                                    "distance": all_dists[c],
                                    "sum_cov_radii": io_ptable.rcov1[
                                        io_ptable.elements.index(symbols[t1])
                                    ]
                                    + io_ptable.rcov1[
            ...
            raise ValueError(
                "Need at least ref_ind, a metal, or atom_pairs, or atom_type_pairs specified!"
            )
    ...
    def functionalize_3D(
        ...
        for i, idx in enumerate(functionalization_inds):
            if isinstance(idx, (int, float)):
                ...
                r = Rot.align_vectors(
                    np.array([[0.0, 0.0, -1.0]] * len(mol_coords)), mol_coords
                )  # .reshape(1,-1))
                ...
                if remove_hydrogens_when_adding[
                    i
                    ...
                    mol_hydrogens_inds = np.intersect1d(
                        np.nonzero(self.graph[idx])[0],
                        np.where(np.array(mol_anums) == 1)[0],
                    )

                    if len(mol_hydrogens_inds) < bond_orders[i]:
                        raise ValueError(
                            "Molecule atom {} bound by fewer hydrogen atoms than needed!".format(
                                idx
                            )
                        )
                    elif len(fg_hydrogens_inds) < bond_orders[i]:
                        raise ValueError(
                            "Functional group atom 0 bound by fewer hydrogen atoms than needed!"
                        )
                    ...
                    fg_hydrogen_dists = np.linalg.norm(
                        funct_coords[fg_hydrogens_inds], axis=1
                    )

                    mol_delete_inds = sorted(
                        mol_hydrogens_inds[
                            mol_hydrogen_dists.argsort()[: bond_orders[i]]
                        ]
                    )[::-1]
                    ...
                    removed_inds = [
                        x + len(np.where(removed_indices <= x)[0])
                        for x in mol_delete_inds
                    ]
                ...
                if funct_mol.charge is not None:
                    self.charge += (
                        funct_mol.charge
                    )  # Add charge for charged functional groups.
                ...
                if not isinstance(
                    remove_hydrogens, (list, np.ndarray)
                ):  # Recast list with default True.
                    remove_hydrogens = [True] * len(functional_group_mol_ind)

                if len(idx) != len(functional_group_mol_ind):
                    raise ValueError(
                        "Functionalization Indices must match functional_group_mol_ind in shape! \n{}!={}!".format(
                ...
                if len(idx) != len(remove_hydrogens):
                    raise ValueError(
                        "Remove hydrogens bools must match functional_group_mol_ind in shape! \n{}!={}!".format(
                ...
                funct_coords = funct_coords - np.mean(
                    funct_coords[functional_group_mol_ind], axis=0
                )
                ...
                for j, fg_mol_ind in enumerate(functional_group_mol_ind):
                    if remove_hydrogens[
                        ...
                        mol_del_inds = mol_hydrogens_inds[
                            mol_hydrogen_dists.argsort()[: bond_orders[i][j]]
                        ].tolist()
                        ...
                        if (
                            ...
                            mol_del_inds = [
                                x
                                for x in mol_hydrogens_inds
                                if x not in mol_delete_inds
                            ][: bond_orders[i][j]]

                        mol_delete_inds += mol_del_inds

                        fg_del_inds = fg_hydrogens_inds[
                            fg_hydrogen_dists.argsort()[: bond_orders[i][j]]
                        ].tolist()
                ...
                for j, fg_mol_ind in enumerate(functional_group_mol_ind):
                    if np.any(remove_hydrogens):
                        tmpind = idx[j] - len(
                            np.where(np.array(mol_delete_inds) <= idx[j])[0]
                        )
                    else:
                        tmpind = idx[j]
                    newligbodict.update(
    ...
    def split_ligs(self):
        ...
        e.g. info_dict for Iron hexaaqua complex.
        info_dict = {
            'metal': 'Fe',
            'metal_ind': 0,
            'lig_charges': [0, 0, 0, 0, 0, 0],
            'lig_coord_ats': ['O', 'O', 'O', 'O', 'O', 'O'],
            'original_lig_inds': [array([1, 2, 3]),
            array([4, 5, 6]),
            array([7, 8, 9]),
            array([10, 11, 12]),
            array([13, 14, 15]),
            array([16, 17, 18])],
            'mapped_smiles_inds': [[0, 1, 2],
            [0, 1, 2],
            [0, 1, 2],
            [0, 1, 2],
            [0, 1, 2],
            [0, 1, 2]],
            'bound_metal_inds': [[0], [0], [0], [0], [0], [0]],
            'lig_smiles': ['O', 'O', 'O', 'O', 'O', 'O'],
            'lig_metal_coordatoms': [[0], [0], [0], [0], [0], [0]]
            }
        """
        (
            ligsmiles,
    ...
    def get_rad_gyration(self):
        ...
        gyration_radii = np.sqrt(
            np.sum(np.linalg.norm((posits - com), axis=1) ** 2 * masses)
            / np.sum(masses)
        )
    ...
    def lig_dissociation_sample(self, min_shift=0.0, max_dist=4, steps=10, do_all=False):
        ...
        Returns
        -------
        output_scans : list(list(str))
            list of lists of mol2strings corresponding to dissociation
            trajectories.
        """
        info_dict = self.split_ligs()
        coords = self.ase_atoms.get_positions()
